...
# Función para mostrar el menú de selección de disco
select_disk() {
    options=()
    while read -r line; do
        options+=("$line")
    done < <(lsblk -d -e 7,11 -o NAME,SIZE,TYPE,MODEL | tail -n +2)

    disk=$(whiptail --title "Seleccionar disco" --menu "Selecciona el disco para la instalación" 15 60 8 "${options[@]}" 3>&1 1>&2 2>&3)
    clear
}

# Mostrar el menú de selección de disco
...
# Función para mostrar el menú de selección de partición raíz
select_partroot() {
    options=()
    while read -r line; do
        options+=("$line")
    done < <(fdisk -l "${drive}" | grep Linux | cut -d" " -f1)

    partroot=$(whiptail --title "Seleccionar partición raíz" --menu "Escoge la partición raíz donde Arch Linux se instalará:" 15 60 8 "${options[@]}" 3>&1 1>&2 2>&3)
    clear
}
...
# Seleccionar partición raíz y formatear
PS3="Escoge la partición raíz que acabas de crear donde Arch Linux se instalará: "
select_partroot

# Formatear la partición raíz y montar
printf "\nFormateando la partición RAIZ %s\nEspera...\n" "${partroot}"
sleep 2
mkfs.ext4 -L Arch "${partroot}" >/dev/null 2>&1
mount "${partroot}" /mnt
sleep 2

echo -e "\t\e[33mPartición raíz formateada y montada en ${partroot}\e[0m"
clear
...
# Confirmación de continuar
while true; do
    sn=$(whiptail --title "Confirmar" --yesno "¿Deseas continuar?" 10 60 3>&1 1>&2 2>&3)
    case $sn in
        0 ) break;;  # Presionó 'Yes'
        1 ) exit;;   # Presionó 'No'
        * ) whiptail --title "Error" --msgbox "Error: Solo necesitas presionar 'Yes' o 'No'" 10 60;;
    esac
done
clear
